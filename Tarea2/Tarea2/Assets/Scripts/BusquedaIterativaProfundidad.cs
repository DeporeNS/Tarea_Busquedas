...
public class BusquedaIterativaProfundidad : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (!foundIt)
        {
            Iterativity(limit.transform.parent);
        }
    }
    ...
    public void Iterativity()
    {


    }

    public void Iterativity(Transform _limit)
    {

        while (que.Count != 0)
        {
            if (que[0].transform.parent != _limit)
            {
                // Buscar a sus hijos.
                for (int i = 0; i < que[0].transform.childCount; i++)
                {
